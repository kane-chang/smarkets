...
export async function fetchEvents(
  ...
  try {
    const response = await fetch(
      `${SMARKETS_API_URL}/events/?state=new&state=upcoming&state=live&type=${type}&with_new_type=true&sort=display_order%2Cstart_datetime%2Cid&limit=50&include_hidden=false`
    );
