...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  const [marketsData, eventData] = await Promise.all([marketsPromise, eventPromise])
    ...
    <main className="p-5 flex flex-col w-full h-full">
      <div className="event-info border-b border-grey-400 ">
        ...
        <div className="flex items-center gap-1 font-semibold text-grey-300 text-xs my-3">
            <IoMdCalendar />
            <p>{eventData.events[0].start_datetime ? formatEventDate(eventData.events[0].start_datetime) : null}</p>
        </div>
      </div>
